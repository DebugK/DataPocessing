...
#
# 고객별로 timestamp가 하나라도 빠져있는 고객을 check 하기위한 grouping
...
#
# # usage 결측치 채우기 (pad + back 하나씩 만들어서 더하고 나누기 2 )
...
#
# # 저장하기 전에!!!
# # meter float ->int64 변경 필요
# # month -> int8 변경 필요
# drop_having_nan_over_24h_df['meter']=drop_having_nan_over_24h_df['meter'].round().astype(np.uint64)
...
# print("=====================================================================")
#
# # 1개의 고객번호에 2개 이상 계기번호를 가진 고객번호 삭제
# duplicated = drop_having_nan_over_24h_df.drop_duplicates(['meter','customer']).groupby(['customer']).size().where(lambda x : x!=1).dropna().index
# drop_having_nan_over_24h_df = drop_having_nan_over_24h_df.loc[~drop_having_nan_over_24h_df['customer'].isin(duplicated)]
#
# # 중복제거를 통한 삭제된 고객번호 비교
# print(drop_having_nan_over_24h_df.nunique())
#
